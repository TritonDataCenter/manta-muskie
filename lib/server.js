...
function createServer(options, clients, name) {
    ...
    //
    // This block exists to run HEAD errors through translateError like we do
    // for other errors (those via formatters).
    //
    // When we're about to return an error due to a failure of a handler, we'll
    // get the 'restifyError' event from:
    //
    // https://github.com/restify/node-restify/blob/6.x/lib/server.js#L1283
    //
    // which is called from either:
    //
    // https://github.com/restify/node-restify/blob/6.x/lib/server.js#L893
    //
    //  - or -
    //
    // https://github.com/restify/node-restify/blob/6.x/lib/server.js#L1047
    //
    // so passing an argument to next() does nothing (since the callback
    // ignores arguments). As such, in order to modify the state of the
    // resulting error, we need to modify 'err' in-place.
    //
    // Note that we listen for `restifyError` here rather than a specific error
    // like 'InternalError' because 'InternalError' might be wrapped (and
    // usually is) in a MultiError. This catches any sort of error.
    //
    server.on('restifyError', function (req, res, err, next) {
        var newErr;

        //
        // We only care about HEAD, because other methods are already handled by
        // the formatters. HEAD is the only *method* where errors won't go
        // through the formatters (which use translateError on errors) because
        // of:
        //
        // https://github.com/restify/node-restify/blob/6.x/lib/response.js#L422
        //
        if (req.method !== 'HEAD') {
            next();
            return;
        ...
        //
        // We need to "translate" the error for HEAD requests the same way we
        // do for other requests. This is especially important in the
        // NoDatabasePeersError + OverloadedError case where moray is
        // overloaded. In that case we translate the error so we return a 503
        // instead of a 500 for all other methods via the formatters, but
        // formatters are ignored for HEAD request, so we want to do that same
        // translation here for those.
        //
        newErr = translateError(err, req);

        log.debug({
            newErr: newErr,
            origErr: err,
            req: req
        }, 'Translating error for HEAD request');

        //
        // We want to keep the original stack rather than having the stack say
        // the error was generated in translateError.
        //
        newErr.stack = err.stack;

        //
        // This is a hack, but unfortunately restify does not allow us to return
        // anything to indicate that we want to change the error. Thus we just
        // replace the error in-place.
        //
        Object.assign(err, newErr);
